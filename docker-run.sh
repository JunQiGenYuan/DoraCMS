...
docker run -it --name $containerName -p $appPort:7878 -d --link $mongoContainerName:mongo --link $redisContainerName:redis $otherArgs registry.cn-hangzhou.aliyuncs.com/simonhg/doracms sh startApp.sh
